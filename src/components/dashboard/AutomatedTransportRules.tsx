...
export const AutomatedTransportRules = ({ profile }: AutomatedTransportRulesProps) => {
  ...
  const [includedRecipients, setIncludedRecipients] = useState(""); // optional whitelist of senders (manual extras)
  const [selectedIncludedUserIds, setSelectedIncludedUserIds] = useState<Set<string>>(new Set()); // registered users picked
  const [includedUserSearch, setIncludedUserSearch] = useState("");
